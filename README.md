...
**Reflektion:**  
MongoDB fungerade bra för snabb och iterativ utveckling med flexibla scheman utan komplicerade migrationer, men kräver mer manuell kontroll av datarelationer.  
Exempelvis måste jag själv validera att `assignedTo` pekar på en existerande användare när tasks skapas, och explicit sätta `assignedTo` till `null` när användare tas bort - saker som SQL med foreign keys hade hanterat automatiskt.  
